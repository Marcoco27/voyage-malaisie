document.addEventListener("DOMContentLoaded", () => {
  ...
  try {
    ...
    voyage.forEach((etape, index) => {
      const nbNuits = etape.nuits || 0; // S'assure qu'on a un nombre
      ...
      notesContainer.innerHTML += `<div class="note-card" id="card-${index}">
          ...
          <div class="card-header"><div class="card-header-text"><h3>${etape.lieu}</h3><span>${etape.dates}</span></div><i class="fas fa-chevron-down card-toggle-icon"></i></div>
          <div class="card-details"><p>${etape.description}</p><strong class="section-title"><i class="fas fa-star"></i>À ne pas manquer</strong><ul>${activitesHTML}</ul>${conseilHTML}</div>
        </div>`;
      
      // Mettre à jour la carte Leaflet
      const nuitLabel = nbNuits === 1 ? 'nuit' : 'nuits';
      const popupContent = `<div class="map-popup">
            <h3>${index + 1}. ${etape.lieu}</h3>
            <p>${etape.description}</p>
            <span><i class="fas fa-moon"></i> ${nbNuits} ${nuitLabel}</span>
        </div>`;
    ...
    const calculateAndDisplayStats = () => {
      const totalNights = voyage.reduce((sum, etape) => sum + (etape.nuits || 0), 0);
      ...
      const totalKms = voyage.reduce((sum, etape) => {
        const kmMatch = etape.distance.toString().match(/(\d+)/); // Extrait le premier nombre trouvé
        return sum + (kmMatch ? parseInt(kmMatch[0], 10) : 0);
      }, 0);
      const uniqueTransportModes = new Set(voyage.flatMap(etape => etape.transport || [])).size;
      
      const animateValue = (element, end, duration) => {
        let start = 0; let startTimestamp = null;
